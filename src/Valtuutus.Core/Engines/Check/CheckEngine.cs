...
public sealed class CheckEngine(IDataReaderProvider reader, Schema schema) : ICheckEngine
{
    ...
    private ValueTask<bool> CheckInternal(CheckRequest req, CancellationToken ct)
    {
    ...
    private ValueTask<bool> CheckPermission(CheckRequest req, Permission permission, CancellationToken ct)
    {
    ...
    private async ValueTask<bool> CheckAttribute(CheckRequest req, CancellationToken ct)
    {
    ...
    private ValueTask<bool> CheckExpression(CheckRequest req, PermissionNode node, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
    ...
    private ValueTask<bool> CheckExpressionChild(CheckRequest req, List<PermissionNode> children, CancellationToken ct, bool isUnion)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();

        var checkFunctions = new List<Func<CancellationToken, ValueTask<bool>>>(capacity: children.Count);
    ...
    private ValueTask<bool> CheckLeaf(CheckRequest req, PermissionNode node, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
        ...
        return node.LeafNode!.Type switch
        {
            ...
            PermissionNodeLeafType.Expression => new ValueTask<bool>(CheckLeafFn(req, node.LeafNode!.ExpressionNode!, ct)),
            _ => throw new InvalidOperationException()
        };
    }
    ...
    private ValueTask<bool> CheckLeafPermission(CheckRequest req, PermissionNodeLeafPermission node, CancellationToken ct)
    {
        if (node.IsIndirect)
    ...
    private ValueTask<bool> CheckComputedUserSet(CheckRequest req, string computedUserSetRelation, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
        return CheckInternal(req with { Permission = computedUserSetRelation }, ct);
    }

    private async ValueTask<bool> CheckTupleToUserSet(CheckRequest req, string tupleSetRelation,
        string computedUserSetRelation, CancellationToken ct)
    {
        ...
        var pool = ArrayPool<Func<CancellationToken, ValueTask<bool>>>.Shared;
    ...
    private async ValueTask<bool> CheckRelation(CheckRequest req, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
    ...
    private static ValueTask<bool> CheckUnion(Func<CancellationToken, ValueTask<bool>>[] functions, int count, CancellationToken ct)
    {
        if (count == 0) return new ValueTask<bool>(false);
        if (count == 1) return functions[0](ct);
        return new ValueTask<bool>(CheckUnionCore(functions, count, ct));
    }

    private static ValueTask<bool> CheckUnion(List<Func<CancellationToken, ValueTask<bool>>> functions, CancellationToken ct)
    {
        if (functions.Count == 0) return new ValueTask<bool>(false);
        if (functions.Count == 1) return functions[0](ct);
        return new ValueTask<bool>(CheckUnionCore(functions, functions.Count, ct));
    }

    private static async Task<bool> CheckUnionCore(IReadOnlyList<Func<CancellationToken, ValueTask<bool>>> functions, int count, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
        ...
        {
            tasks[i] = functions[i](cancellationToken).AsTask().ContinueWith(
                static (t, state) =>
                {
    ...
    private static ValueTask<bool> CheckIntersect(List<Func<CancellationToken, ValueTask<bool>>> functions, CancellationToken ct)
    {
        if (functions.Count == 0) return new ValueTask<bool>(true);
        if (functions.Count == 1) return functions[0](ct);
        return new ValueTask<bool>(CheckIntersectCore(functions, ct));
    }

    private static async Task<bool> CheckIntersectCore(List<Func<CancellationToken, ValueTask<bool>>> functions, CancellationToken ct)
    {
        using var activity = DefaultActivitySource.InternalSourceInstance.StartActivity();
        using var pooledCts = CancellationTokenSourcePool.Rent(ct);
        var cancellationToken = pooledCts.Token;
