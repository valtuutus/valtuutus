...
public class LookupEngineV2(
    Schema schema,
    ILogger<LookupEngineV2> logger,
    ...
    IAttributeReader attributeReader)
{
    ...
    private LookupFunction LookupRelation(LookupEntityRequestInternal req, Schemas.Relation relation)
    {
        return async (ct) =>
        {
            ...
            // logger.LogDebug("Looking up relation {relation} with req: {req}", req.Permission, req);
            ...
            foreach (var relationEntity in relation.Entities)
            {
                ...
                if (subRelation is not null)
                {
                    ...
                    lookupFunctions.Add((ct) => CheckJoin(main, dependent, ct));
                }
            }
    ...
    private async Task<List<RelationOrAttributeTuple>> CheckJoin(
